...
public class NoticeTargetController {
    ...
    @ApiOperation("查询通知内容")
    @GetMapping
//    @PreAuthorize("@smpe.check('noticeTarget:list')")
    @AnonymousAccess
    public Result<Object> query(NoticeTargetQueryCriteria criteria, PageVO pageVO) {
//        log.info(StrUtil.format("【查询通知内容 /api/noticeTarget】操作人id：{}，通知内容查询条件 criteria：{}", SecurityUtils.getCurrentUserId(),
//                criteria));
...
//        log.info(StrUtil.format("【新增通知内容 /api/noticeTarget】操作人id：{}，通知内容：{}", SecurityUtils.getCurrentUserId(),
...
//        log.info(StrUtil.format("【修改通知内容 /api/noticeTarget】操作人id：{}，被修改通知内容id：{}", SecurityUtils.getCurrentUserId(),
//                resources.getId()));
...
//    @PreAuthorize("@smpe.check('noticeTarget:del')")
    ...
    public Result<Object> delete(@RequestBody Set<Long> ids) {
        log.info(StrUtil.format("【删除通知内容 /api/noticeTarget】操作人id：{}，被删除通知内容id：{}", SecurityUtils.getCurrentUserId(), ids.toString()));
        // 验证是否被用户关联
