...
public class RoleServiceImpl extends BasicServiceImpl<RoleMapper, Role> implements IRoleService {
    ...
    public RoleDTO findById(Long roleId) {
        ...
        if (ObjectUtil.isEmpty(roleBO)) {
            log.error("【查询角色失败】用户名不存在。操作人id：{}，角色id：{}", SecurityUtils.getCurrentUserId(),
                    roleId);
    ...
    public void insertRole(RoleInsertOrUpdateDTO roleInsertOrUpdateDTO) {
        ...
        if (isExistRoleName(roleInsertOrUpdateDTO.getName())) {
            log.error("【新增角色失败】新增角色名已存在。操作人id：{}，新增角色名：{}", SecurityUtils.getCurrentUserId(),
        ...
        if (!save) {
            log.error("【新增角色失败】操作人id：{}，新增角色名：{}", SecurityUtils.getCurrentUserId(),
                    roleInsertOrUpdateDTO.getName());
        ...
        if (roleInsertOrUpdateDTO.getDataScope().equals(DataScopeEnum.CUSTOMIZE.getValue())) {
            if (CollectionUtil.isNotEmpty(roleInsertOrUpdateDTO.getDepts())) {
                ...
                if (count <= 0) {
                    log.error("【新增角色失败】维护角色部门中间表失败。操作人id：{}，角色id：{}", SecurityUtils.getCurrentUserId(),
                            role.getId());
        ...
        log.info("【新增角色成功】操作人id：{}，角色id：{}", SecurityUtils.getCurrentUserId(),
                role.getId());
    }
    ...
    public void updateRole(RoleInsertOrUpdateDTO roleInsertOrUpdateDTO) {
        ...
        if (ObjectUtil.isEmpty(roleBO)) {
            log.error("【修改角色信息失败】此角色不存在。操作人id：{}，修改角色id：{}", SecurityUtils.getCurrentUserId(),
        ...
        if (!roleInsertOrUpdateDTO.getName().equals(roleBO.getName())) {
            if (isExistRoleName(roleInsertOrUpdateDTO.getName())) {
                log.error("【修改角色失败】角色名已存在。操作人id：{}，角色名：{}", SecurityUtils.getCurrentUserId(),
                        roleInsertOrUpdateDTO.getName());
                throw new BadRequestException(ResultEnum.ROLE_NAME_EXIST);
            }
        }
...
//                    }
            ...
            log.error("【修改角色失败】维护角色部门表失败。操作人id：{}，修改角色id：{}", SecurityUtils.getCurrentUserId(),
                    roleInsertOrUpdateDTO.getId());
            throw new BadRequestException(ResultEnum.OPERATION_MIDDLE_FAIL);
        }
        ...
        if (!isUpdate) {
            log.error("【修改角色失败】操作人id：{}，修改角色id：{}", SecurityUtils.getCurrentUserId(),
                    roleInsertOrUpdateDTO.getId());
        }

        log.info("【修改角色成功】操作人id：{}，修改角色id：{}", SecurityUtils.getCurrentUserId(),
                roleInsertOrUpdateDTO.getId());
        // 更新相关缓存
    ...
    public void updateMenu(Long roleId, Set<Long> menuIds) {
        ...
        if (!CollectionUtils.isEqualCollection(menuIds, oldMenuIds)) {
            ...
            if (count <= 0 && count2 <= 0) {
                log.error("【修改角色菜单失败】维护角色菜单表失败。操作人id：{}，修改角色id：{}", SecurityUtils.getCurrentUserId(),
                        roleId);
                throw new BadRequestException(ResultEnum.OPERATION_MIDDLE_FAIL);
            }

            log.info("【修改角色菜单成功】操作人id：{}，修改角色id：{}", SecurityUtils.getCurrentUserId(),
                    roleId);
        }
        delCaches(roleId);
    }
    ...
    public boolean isRolesWithUser(Set<Long> roleIds) {
        if (userMapper.countByUserByRoleIds(roleIds) > 0) {
            log.info("【角色与用户存在绑定关系】操作人id：{}，角色id集合：{}", SecurityUtils.getCurrentUserId(), roleIds);
            return true;
        }
        return false;
    }
    ...
    public void delete(Set<Long> roleIds) {
        ...
        if (!isDel) {
            log.error("【删除角色失败】操作人id：{}，角色id集合：{}", SecurityUtils.getCurrentUserId(), roleIds);
