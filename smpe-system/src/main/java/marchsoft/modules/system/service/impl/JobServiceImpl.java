...
public class JobServiceImpl extends BasicServiceImpl<JobMapper, Job> implements IJobService {
    ...
    public JobDTO findById(Long id) {
        ...
        if (ObjectUtil.isEmpty(job)) {
            log.error("【查询岗位失败】操作人id：{}，查询岗位id：{}", SecurityUtils.getCurrentUserId(),
                    id);
    ...
    public void download(List<JobDTO> jobDtos, HttpServletResponse response) throws IOException {
        if (CollectionUtil.isEmpty(jobDtos)) {
            log.error("【导出岗位失败】操作人id：{}，导出数据集合为空jobDtos：{}", SecurityUtils.getCurrentUserId(),
                    jobDtos);
    ...
    public void create(Job resources) {
        ...
        if (count(queryWrapper) > 0) {
            log.error("【创建岗位失败】操作人id：{}，创建目标（名称）已存在 job：{}", SecurityUtils.getCurrentUserId(),
                    resources);
            throw new BadRequestException("已存在：" + resources.getName());
        }
        save(resources);
        log.info("【创建岗位成功】操作人id：{}，创建目标job：{}", SecurityUtils.getCurrentUserId(),
                resources);
    }
    ...
    public void update(Job resources) {
        ...
        if (ObjectUtil.isEmpty(job)) {
            log.error("【修改岗位失败】操作人id：{}，修改目标job为空，目标jobId：{}", SecurityUtils.getCurrentUserId(),
                    resources.getId());
        ...
        if (ObjectUtil.isNotNull(old) && !old.getId().equals(resources.getId())) {
            log.error("【修改岗位失败】操作人id：{}，修改目标（名称）已存在，修改目标job：{}，存在目标job：{}", SecurityUtils.getCurrentUserId(),
                    resources, old);
            throw new BadRequestException("已存在：" + resources.getName());
        }
        ...
        log.info("【修改岗位成功】操作人id：{}，修改目标job：{}", SecurityUtils.getCurrentUserId(),
                resources);
    }
    ...
    public void delete(Set<Long> ids) {
        ...
        removeByIds(ids);
        log.info("【删除岗位成功】操作人id：{}，删除目标jobs：{}", SecurityUtils.getCurrentUserId(),
                ids.toString());
    }
    ...
    public void verification(Set<Long> ids) {
        if (userMapper.countByJobs(ids) > 0) {
            log.error("【删除岗位失败】操作人id：{}，所选的岗位：{}中存在用户关联，请解除关联再试！", SecurityUtils.getCurrentUserId(),
                    ids.toString());
